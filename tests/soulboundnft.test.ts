...
describe("SellSoulboundNFT", function () {
    ...
    describe("Whitelist mint", () => {
        it("should allow whitelist mint", async () => {
            ...
            const mint = await contract.connect(user1).buy(paymentToken1Address, proof, true);
            await expect(mint).to.emit(contract, "NFTSold");
        });
        it("should mint successfully with valid proof", async () => {
            const proof = merkleTree.getProof(0);
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            await expect(contract.connect(user1).buy(paymentToken1Address, proof, true)).to.emit(contract, "NFTSold");
        });
        ...
        it("should fail with invalid proof", async () => {
            ...
            await expect(
                contract.connect(user2).buy(paymentToken1Address, wrongProof, true),
        ...
        it("should fail if already minted", async () => {
            const proof = merkleTree.getProof(0);
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            await contract.connect(user1).buy(paymentToken1Address, proof, true);
            await expect(contract.connect(user1).buy(paymentToken1Address, proof, true)).to.be.reverted;
        });
    });
    ...
    describe("Public mint", () => {
        it("should allow public mint", async () => {
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            const mint = await contract.connect(user1).buy(paymentToken1Address, [], false);
            await expect(mint).to.emit(contract, "NFTSold");
        });
    });
    ...
    describe("Payment tokens", () => {
        it("should fail with unsupported token", async () => {
            ...
            await expect(contract.connect(user1).buy(randomTokenAddress, [], false)).to.be.revertedWithCustomError(
                contract,
                "InvalidPaymentToken",
            );
        });
    ...
    describe("Buy with multiple payment tokens", () => {
        ...
        it("should allow buy with first token (USDT1)", async () => {
            ...
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            await contract.connect(user1).buy(paymentToken1Address, proofUser1, true);
        ...
        it("should allow buy with second token (USDT2)", async () => {
            ...
            await contract.connect(user1).buy(paymentToken2Address, proofUser1, true);
        ...
        it("should allow multiple buys with different tokens", async () => {
            ...
            await contract.connect(user1).buy(paymentToken1Address, [], false);
            await contract.connect(user2).buy(paymentToken2Address, [], false);
        ...
        it("should revert if payment token is not supported", async () => {
            ...
            await expect(contract.connect(user1).buy(fakeTokenAddress, [], false)).to.be.revertedWithCustomError(
                contract,
                "InvalidPaymentToken",
            );
        });
    ...
    describe("Pause", () => {
        it("should pause and unpause correctly", async () => {
            ...
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            await expect(contract.connect(user1).buy(paymentToken1Address, [], false)).to.be.reverted;

            await contract.connect(owner).unpause();
            await expect(contract.connect(user1).buy(paymentToken1Address, [], false)).to.emit(contract, "NFTSold");
        });
        ...
        it("should block mint when paused", async () => {
            ...
            const proof = merkleTree.getProof(0);
            await fundAndApprove(user1, paymentToken1, ethers.parseEther("10"));
            await expect(contract.connect(user1).buy(paymentToken1Address, proof, true)).to.be.revertedWith(
                "Pausable: paused",
            );
        });
    });
    ...
    it("should reject invalid proof", async () => {
        const proof = merkleTree.getProof(0);
        await expect(contract.connect(user2).buy(paymentToken1Address, proof, true)).to.be.revertedWithCustomError(
            contract,
            "InvalidProof",
        );
    });
